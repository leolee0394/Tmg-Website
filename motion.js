/* Scroll choreography (GSAP + ScrollTrigger). Each block only runs if its elements exist on the page. */
(function () {
  'use strict';
  ...
  function init() {
    ...
    function paint(s, dur) { gsap.to(root, { '--pagebg': res(s.dataset.bg), '--pagefg': res(s.dataset.fg || 'var(--ink)'), duration: dur, overwrite: 'auto' }); }
    var secs = $$('[data-bg]');
    secs.forEach(function (s) {
      ScrollTrigger.create({ trigger: s, start: 'top 55%', end: 'bottom 55%', onToggle: function (self) { if (self.isActive) paint(s, .6); } });
    });
    var first = secs.filter(function (s) { var b = s.getBoundingClientRect(); return b.top < innerHeight * .55 && b.bottom > innerHeight * .55; })[0];
    if (first) paint(first, 0);
    ...
    if (net) {
      ...
      gsap.timeline({ scrollTrigger: { trigger: net, start: 'top 80%', end: 'bottom 55%', scrub: .8 } })
  ...
  /* opening story: a seed splits into Food + Medicine, then interlocks */
  function hero(reduce) {
    var root = document.documentElement, N = 16, TAU = Math.PI * 2;
    ...
    if (reduce) { root.classList.add('reduce'); Object.assign(A, leaf(300), mergedA); Object.assign(B, mergedB); draw(); return; }

    gsap.from('#grow', { scale: .25, opacity: 0, svgOrigin: '500 500', duration: 1.8, ease: 'power3.out' });
    gsap.set('.cap2,.cap3', { autoAlpha: 0 });
    var tl = gsap.timeline({ defaults: { ease: 'none' }, onUpdate: draw,
